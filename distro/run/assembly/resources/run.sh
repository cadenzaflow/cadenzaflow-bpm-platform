...
OPTIONS_HELP="Options:
  --webapps    - Enables the CadenzaFlow Platform Webapps
  --oauth2     - Enables the CadenzaFlow Platform Spring Security OAuth2 integration
  ...
  --detached   - Starts CadenzaFlow Run as a detached process
"
...
if [ "$1" = "start" ] ; then
  ...
  while [ "$1" != "" ]; do
    case $1 in
      ...
      --detached )   detachProcess=true
                     echo CadenzaFlow Run will start in the background. Use the shutdown.sh script to stop it
                     ;;
  ...
  if [ "$detachProcess" = "true" ]; then
    ...
    # check if a CadenzaFlow Run instance is already in operation
    if [ -s "$PID_PATH" ]; then
      echo "
A CadenzaFlow Run instance is already in operation (process id $(cat $PID_PATH)).

Please stop it or remove the file $PID_PATH."
      exit 1
    fi

    # start CadenzaFlow Run detached
    "$JAVA" -Dloader.path="$classPath" -Dcadenzaflow.deploymentDir="$DEPLOYMENT_DIR" $JAVA_OPTS -jar "$BASEDIR/cadenzaflow-bpm-run-core.jar" --spring.config.location=file:"$configuration" &
  ...
  if [ -s "$PID_PATH" ]; then
    # stop CadenzaFlow Run if the process is still running
    kill $(cat "$PID_PATH")
    ...
    echo "There is no instance of CadenzaFlow Run to shut down."
    exit 1
  fi
