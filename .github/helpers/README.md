...
1. Sync the label with `cadenzaflow/cadenzaflow-bpm-platform`:
    1. Set an environment variable `GITHUB_ACCESS_TOKEN` with your github token
    1. `github-label-sync --dry-run --labels ./.github/pr-labels.yml --allow-added-labels cadenzaflow/cadenzaflow-bpm-platform`
...
1. Sync the labels by applying the same procedure with repositories:
    * `cadenzaflow/cadenzaflow-bpm-platform-maintenance`
    * `cadenzaflow/cadenzaflow-bpm-platform-ee`
    * `cadenzaflow/cadenzaflow-bpm-platform-ee-maintenance`
    * `cadenzaflow/cadenzaflow-bpm-rpa-bridge-ee`
  
Read https://confluence.cadenzaflow.com/display/AP/Github+labels for how to change other labels and how we overall work with labels.